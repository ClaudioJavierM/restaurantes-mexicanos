...
use App\Jobs\SendUnclaimedCouponEmailJob;
use App\Models\EmailSuppression;
use App\Models\Restaurant;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Log;

class SendUnclaimedCouponEmails extends Command
{
    protected $signature = 'famer:send-unclaimed-coupon
                            {--limit=200 : Maximum number of emails to dispatch}
                            {--dry-run : Preview how many would be dispatched without sending}';

    protected $description = 'Send quarterly $1 Premium coupon emails to unclaimed restaurants';
    ...
    public function handle(): int
    {
        ...
        $query = Restaurant::query()
            ...
            ->where(function ($q) {
                // Quarterly: send if never sent, or last sent more than ~80 days ago
                $q->whereNull('unclaimed_coupon_sent_at')
                  ->orWhere('unclaimed_coupon_sent_at', '<', now()->subDays(80));
            })
            ->limit($limit);
        ...
        if ($dryRun) {
            $this->info("[dry-run] Would dispatch quarterly coupon emails for {$count} restaurants (limit: {$limit}).");
            return Command::SUCCESS;
        }

        $i = 0;
        foreach ($toDispatch as $restaurant) {
            SendUnclaimedCouponEmailJob::dispatch($restaurant->id)
                ->delay(now()->addSeconds($i * 2)); // 2-second stagger
            $i++;
        }

        $this->info("Dispatched quarterly coupon emails for {$count} restaurants.");
        Log::info("famer:send-unclaimed-coupon dispatched {$count} jobs (limit: {$limit}).");
