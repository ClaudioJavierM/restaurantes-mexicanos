...
@section('title')Best {{ $data['title_en'] ?? $dishName . ' Restaurants' }} | FAMER@endsection
@section('meta_description'){{ $data['description_en'] ?? $data['description'] }}@endsection

@push('meta')
<meta property="og:type" content="website">
<meta property="og:url" content="{{ url()->current() }}">
<meta property="og:title" content="Best {{ $data['title_en'] ?? $dishName . ' Restaurants' }} | FAMER">
<meta property="og:description" content="{{ $data['description_en'] ?? $data['description'] }}">

<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{{ $data['title_en'] ?? $dishName . ' Restaurants' }} | FAMER">
<meta name="twitter:description" content="{{ $data['description_en'] ?? $data['description'] }}">
@endpush
...
<div class="bg-[#0B0B0B] border-b border-[#D4AF37]/20">
    <div class="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        <nav class="text-sm mb-6 flex items-center gap-2 text-gray-500">
            ...
            <a href="{{ url('/restaurants') }}" class="hover:text-[#D4AF37] transition-colors">Restaurants</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        ...
        <h1 class="text-3xl md:text-5xl font-bold text-[#F5F5F5] mb-4" style="font-family: 'Playfair Display', serif;">
            Best <span class="text-[#D4AF37]">{{ $dishName }}</span> Restaurants
        </h1>
        <p class="text-lg text-gray-400 max-w-3xl">
            Find authentic Mexican restaurants serving <strong class="text-[#F5F5F5]">{{ $dishName }}</strong> near you.
        ...
        <div class="flex gap-4 mt-8 flex-wrap">
            <div class="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl px-6 py-4 text-center">
                ...
                <p class="text-sm text-gray-400 mt-1">Restaurants</p>
            </div>
            <div class="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl px-6 py-4 text-center">
                <p class="text-3xl font-bold text-[#D4AF37]">Authentic</p>
                <p class="text-sm text-gray-400 mt-1">Mexican Cuisine</p>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <aside class="lg:col-span-1 order-last lg:order-first">
                <div class="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6 sticky top-24 space-y-6">
                    ...
                    {{-- Other dishes --}}
                    <div>
                        <h3 class="font-bold text-[#F5F5F5] mb-3 text-sm uppercase tracking-wider">Other Dishes</h3>
                        <ul class="space-y-1">
                            @foreach([
                                'birria'          => 'Birria',
                                'tacos'           => 'Mexican Tacos',
                    ...
                    {{-- CTA --}}
                    <div>
                        <h3 class="font-bold text-[#F5F5F5] mb-2 text-sm uppercase tracking-wider">Restaurant Owner?</h3>
                        <p class="text-sm text-gray-400 mb-4">Claim your restaurant and showcase your menu on FAMER.</p>
                        <a href="{{ url('/for-owners') }}"
            ...
            <main class="lg:col-span-3">
                ...
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-[#F5F5F5]" style="font-family: 'Playfair Display', serif;">
                        Best {{ $dishName }} Restaurants
                    </h2>
                </div>
                <p class="text-gray-400 text-sm mb-6">
                    {{ number_format($totalCount) }} restaurants · Sorted by rating.
                </p>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <div class="flex gap-1 flex-shrink-0">
                                @if($isElite)
                                    <span class="bg-[#D4AF37]/10 text-[#D4AF37] text-xs px-2 py-0.5 rounded-full border border-[#D4AF37]/30 font-semibold whitespace-nowrap">Featured</span>
                                @elseif($isPremium)
                                ...
                                @if($restaurant->is_claimed)
                                    <span style="background:rgba(212,175,55,0.06); color:#D4AF37; border:1px solid rgba(212,175,55,0.15);" class="text-xs px-2 py-0.5 rounded-full whitespace-nowrap">Verified</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 mt-1.5">
                            ...
                            <span class="text-xs text-gray-500">({{ number_format($reviews) }} reviews)</span>
                        </div>
                        @endif
                ...
                <div class="text-center py-16 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl">
                    ...
                    <p class="text-gray-400">No restaurants found for <strong class="text-[#F5F5F5]">{{ $dishName }}</strong> yet.</p>
                    <p class="text-sm text-gray-500 mt-2">Own a restaurant? <a href="{{ url('/for-owners') }}" class="text-[#D4AF37] hover:underline">Join FAMER free</a></p>
                </div>
                @endforelse
                ...
                <section class="mt-12 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-8">
                    <h2 class="text-xl font-bold text-[#F5F5F5] mb-4" style="font-family: 'Playfair Display', serif;">
                        Where to find authentic {{ $dishName }}?
                    </h2>
                    <div class="text-gray-400 space-y-4 text-sm leading-relaxed">
                        <p>{{ $data['body'] }}</p>
                        <p>
                            Our guide includes <strong class="text-[#F5F5F5]">{{ number_format($totalCount) }} verified restaurants</strong>
                            serving authentic {{ strtolower($dishName) }}. All ratings come from Google and are periodically verified.
                        </p>
                    </div>
                    ...
                    <div class="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                        @foreach(['tx'=>'Texas','ca'=>'California','il'=>'Illinois','az'=>'Arizona','fl'=>'Florida','co'=>'Colorado','nv'=>'Nevada','nm'=>'New Mexico','ny'=>'New York','wa'=>'Washington'] as $code => $stateName)
                        <a href="{{ url('/' . $dish . '-in-' . $code) }}"
                    ...
                    {{-- Owner CTA --}}
                    <div class="mt-8 border-t border-[#2A2A2A] pt-6 flex flex-col sm:flex-row items-center gap-4 justify-between">
                        <div>
                            <p class="text-sm font-semibold text-[#F5F5F5]">Are you a restaurant owner?</p>
                            <p class="text-xs text-gray-500 mt-0.5">Join FAMER free and reach thousands of food lovers searching for authentic Mexican cuisine.</p>
                        </div>
                        <a href="{{ url('/for-owners') }}"
                           class="flex-shrink-0 bg-[#D4AF37] text-[#0B0B0B] px-6 py-2.5 rounded-lg font-bold text-sm hover:bg-[#E8C67A] transition-colors whitespace-nowrap">
                            Join FAMER Free
                        </a>
                    </div>
                </section>
...
<script type="application/ld+json">
{
    ...
    "@@type": "ItemList",
    "name": "{{ addslashes($data['title_en'] ?? $data['title']) }}",
    "description": "{{ addslashes($data['description_en'] ?? $data['description']) }}",
    "inLanguage": "en",
