...
@section('title')
{{ $dishData['name'] }} en {{ $stateName }} — Restaurantes Mexicanos | FAMER
@endsection

@section('meta_description')Los mejores restaurantes de {{ $dishData['name'] }} auténtica en {{ $stateName }}. Descubre {{ $restaurants->count() }} lugares verificados en FAMER.@endsection
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav style="margin-bottom:1.5rem;">
                <ol style="display:flex; flex-wrap:wrap; gap:0.5rem; align-items:center; font-size:0.875rem; color:#9CA3AF; list-style:none; padding:0; margin:0;">
                    ...
                    <li style="color:#4B5563;">/</li>
                    <li style="color:#9CA3AF;">{{ $stateName }}</li>
                </ol>
            </nav>
            <div style="text-align:center;">
                <h1 style="font-family:'Playfair Display',serif; font-size:clamp(2rem,5vw,3.5rem); font-weight:700; color:#F5F5F5; margin-bottom:1rem; line-height:1.2;">
                    {{ $dishData['name'] }} en {{ $stateName }}
                </h1>
                <p style="color:#9CA3AF; font-size:1.125rem; max-width:600px; margin:0 auto 2rem;">
                    Los mejores restaurantes de {{ $dishData['name'] }} auténtica en {{ $stateName }}. {{ $restaurants->count() }} lugares verificados.
                </p>
                <div style="display:flex; gap:0.75rem; justify-content:center; flex-wrap:wrap;">
                    ...
                    <a href="/{{ $dish }}" style="background:#1A1A1A; border:1px solid #2A2A2A; color:#9CA3AF; padding:0.5rem 1.25rem; border-radius:9999px; font-size:0.875rem; text-decoration:none;">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                <h2 style="font-family:'Playfair Display',serif; font-size:1.75rem; font-weight:700; color:#F5F5F5; margin-bottom:1.5rem; padding-bottom:0.75rem; border-bottom:1px solid #2A2A2A;">
                    Mejores {{ $dishData['name'] }} en {{ $stateName }}
                </h2>
                <div style="display:flex; flex-direction:column; gap:1rem;">
                    ...
                    <a href="/restaurante/{{ $restaurant->slug }}"
                       ...
                       onmouseover="this.style.borderColor='#D4AF37'" onmouseout="this.style.borderColor='#2A2A2A'">
                        ...
                        <div style="flex:1; min-width:0;">
                            ...
                            <p style="font-size:0.875rem; color:#9CA3AF; margin:0 0 0.25rem;">{{ $restaurant->city }}</p>
                    ...
                    @empty
                    <p style="color:#9CA3AF; text-align:center; padding:3rem 0;">No encontramos restaurantes en este estado aún.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div style="background:#1A1A1A; border:1px solid #2A2A2A; border-radius:12px; padding:1.5rem; position:sticky; top:1rem;">
                    <h3 style="font-size:1rem; font-weight:600; color:#F5F5F5; margin-bottom:1rem;">{{ $dishData['name'] }} en Otros Estados</h3>
        ...
        <div style="margin-top:4rem; padding-top:3rem; border-top:1px solid #2A2A2A;">
            <h2 style="font-family:'Playfair Display',serif; font-size:1.5rem; font-weight:700; color:#F5F5F5; margin-bottom:1rem;">
                {{ $dishData['name'] }} Auténtica en {{ $stateName }}
            </h2>
...
<script type="application/ld+json">
{
    ...
    "description": "Los mejores restaurantes de {{ addslashes($dishData['name']) }} en {{ addslashes($stateName) }}",
