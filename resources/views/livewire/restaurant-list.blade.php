                            ...
                            <a href="{{ route('restaurants.show', $restaurant->slug) }}"
                               ...
                               @mouseenter="$dispatch('highlight-marker', { index: {{ $index }} })">
                                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-200 relative">
                                    ...
                                    <!-- Favorite Heart -->
                                    <button
                                        wire:click.prevent.stop="toggleFavorite({{ $restaurant->id }})"
                                        class="absolute top-3 right-3 z-10 w-9 h-9 rounded-full flex items-center justify-center shadow-lg transition-all duration-200"
                                        style="background:rgba(0,0,0,0.6); backdrop-filter:blur(4px);"
                                        title="{{ in_array($restaurant->id, $userFavoriteIds) ? 'Quitar de favoritos' : 'Guardar en favoritos' }}"
                                    >
                                        <svg class="w-5 h-5" fill="{{ in_array($restaurant->id, $userFavoriteIds) ? '#D4AF37' : 'none' }}" stroke="{{ in_array($restaurant->id, $userFavoriteIds) ? '#D4AF37' : '#ffffff' }}" viewBox="0 0 24 24">
